...
- **Cmd+P file jumper** — press `Cmd+P` (macOS) or `Ctrl+P` (Windows/Linux) to open a VS Code-style quick-open palette. Type to filter files by path (substring, case-insensitive, up to 10 results). `Enter` or click to jump to a file and scroll to it; `Escape` or click outside to close; `↑` / `↓` to navigate. Closes the GitHub `t` file-finder + VS Code `Cmd+P` quick-open gap with a palette that filters the current round's file tree.
- **Submit confirm modal** — clicking "Submit Review" now shows a confirmation modal with the open finding count: "You're about to submit your review. Review N open findings before submitting." `Cancel` / `Escape` / click outside closes and returns; `Submit` proceeds to the existing submit flow. Prevents accidental submit with work still in progress. Closes the GitHub "Submit review" modal + Gerrit "Submit Patch Set" confirm gap.
- **Comments audit trail** — when you edit a finding's category, severity, or comment via the in-place Edit button, the prior version is preserved as an audit row (`{ before, after, at, by }`) in the finding's comment thread. A collapsible "N edits — click to expand" disclosure renders the history inline. The agent's R15 honor directive treats the most recent `audit_log` entry as the user's current intent (not your prior fix attempt). `FindingAuditRow` is additive optional (`audit_log?: FindingAuditRow[]`) — existing `state.json` files from R12–R14 load without migration. Closes the Phabricator Differential audit trail gap.
...
- **Keyboard shortcuts** (work in the review UI):
  ...
  - `Cmd+P` (or `Ctrl+P` on Windows/Linux) — open the file jumper palette to jump to any file by name (R15 #26). Type to filter, `Enter` to jump, `Escape` to close.
